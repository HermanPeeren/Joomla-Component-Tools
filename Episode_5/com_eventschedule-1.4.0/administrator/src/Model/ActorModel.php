...
class ActorModel extends AdminModel
{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_eventschedule.edit.actor.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
				...
				{
					if (is_array($table->events))
					{
						$table->events = implode(',', $table->events);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__eventschedule_actors');
				$max             = $db->loadResult();
