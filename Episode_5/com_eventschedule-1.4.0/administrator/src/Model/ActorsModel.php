...
class ActorsModel extends ListModel
{
	...
	*/
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__eventschedule_actors` AS a');
		
		// Join over the users for the checked out user
		$query->select("uc.name AS uEditor");
		$query->join("LEFT", "#__users AS uc ON uc.id=a.checked_out");
		// Join over the foreign key 'events'
		$query->select('`#__eventschedule_events_4107503`.`event_name` AS events_fk_value_4107503');
		$query->join('LEFT', '#__eventschedule_events AS #__eventschedule_events_4107503 ON #__eventschedule_events_4107503.`id` = a.`events`');
		// Join over the foreign key 'schedule'
		$query->select('`#__eventschedule_eventschedules_4107540`.`title` AS eventschedules_fk_value_4107540');
		$query->join('LEFT', '#__eventschedule_eventschedules AS #__eventschedule_eventschedules_4107540 ON #__eventschedule_eventschedules_4107540.`id` = a.`schedule`');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.actor_name LIKE ' . $search . ' )');
			}
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->events))
			{
				$values    = explode(',', $oneItem->events);
				$textValue = array();
				...
				foreach ($values as $value)
				{
					...
					$query
						...
						->where($db->quoteName('#__eventschedule_events_4107503.id') . ' = '. $db->quote($db->escape($value)));
				...
				$oneItem->events = !empty($textValue) ? implode(', ', $textValue) : $oneItem->events;
			}
			...
			if (isset($oneItem->schedule))
			{
				...
				foreach ($values as $value)
				{
					...
					$query
						...
						->where($db->quoteName('#__eventschedule_eventschedules_4107540.id') . ' = '. $db->quote($db->escape($value)));
