...
class EventtypesModel extends ListModel
{
	...
	*/
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__eventschedule_event_types` AS a');
		...
		$query->select('`#__eventschedule_eventschedules_4107542`.`title` AS eventschedules_fk_value_4107542');
		$query->join('LEFT', '#__eventschedule_eventschedules AS #__eventschedule_eventschedules_4107542 ON #__eventschedule_eventschedules_4107542.`id` = a.`schedule`');
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->schedule))
			{
				...
				foreach ($values as $value)
				{
					...
					$query = $db->getQuery(true);
					$query
						->select('`#__eventschedule_eventschedules_4107542`.`title`')
						->from($db->quoteName('#__eventschedule_eventschedules', '#__eventschedule_eventschedules_4107542'))
						->where($db->quoteName('#__eventschedule_eventschedules_4107542.id') . ' = '. $db->quote($db->escape($value)));
