...
class SectionModel extends AdminModel
{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_eventschedule.edit.section.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
				...
				if (!empty($table->container_ids))
				{
					if (is_array($table->container_ids))
					{
						$table->container_ids = implode(',', $table->container_ids);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__eventschedule_sections');
				$max             = $db->loadResult();
