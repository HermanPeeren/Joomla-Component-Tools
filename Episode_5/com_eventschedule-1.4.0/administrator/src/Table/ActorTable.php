 ...
 * @since 1.4.0
 */
class ActorTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for multiple or not foreign key field: events
			if(!empty($array['events']))
			{
				if(is_array($array['events'])){
					$array['events'] = implode(',',$array['events']);
				}
				else if(strrpos($array['events'], ',') != false){
					$array['events'] = explode(',',$array['events']);
				}
			}
			else {
				$array['events'] = 0;
		...
		if (!$user->authorise('core.admin', 'com_eventschedule.actor.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_eventschedule/access.xml',
				"/access/section[@name='actor']/"
			);
			$default_actions = Access::getAssetRules('com_eventschedule.actor.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		// Support for subform field actor_image
		if (is_array($this->actor_image))
		{
			$this->actor_image = json_encode($this->actor_image, JSON_UNESCAPED_UNICODE);
		}
