 ...
 * @since 1.4.0
 */
class SectionTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for multiple or not foreign key field: container_ids
			if(!empty($array['container_ids']))
			{
				if(is_array($array['container_ids'])){
					$array['container_ids'] = implode(',',$array['container_ids']);
				}
				else if(strrpos($array['container_ids'], ',') != false){
					$array['container_ids'] = explode(',',$array['container_ids']);
				}
			}
			else {
				$array['container_ids'] = 0;
			}
		...
		if (!$user->authorise('core.admin', 'com_eventschedule.section.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_eventschedule/access.xml',
				"/access/section[@name='section']/"
			);
			$default_actions = Access::getAssetRules('com_eventschedule.section.' . $array['id'])->getData();
			$array_jaccess   = array();
