...
<form action="<?php echo Route::_('index.php?option=com_eventschedule&view=containers'); ?>" method="post"
			...
			<?php echo LayoutHelper::render('joomla.searchtools.default', array('view' => $this)); ?>
				...
				<table class="table table-striped" id="containerList">
					...
					<th  scope="col" class="w-1 text-center">
						...
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_EVENTSCHEDULE_CONTAINERS_CONTAINER_NAME', 'a.container_name', $listDirn, $listOrder); ?>
						</th>
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_EVENTSCHEDULE_CONTAINERS_SCHEDULE', 'a.schedule', $listDirn, $listOrder); ?>
						</th>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'containers.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'containers.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo Route::_('index.php?option=com_eventschedule&task=container.edit&id='.(int) $item->id); ?>">
									<?php echo $this->escape($item->container_name); ?>
									</a>
								<?php else : ?>
												<?php echo $this->escape($item->container_name); ?>
								<?php endif; ?>
